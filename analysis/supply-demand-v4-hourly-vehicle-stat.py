...
for av_i, av in enumerate(av_list):
    ...
    for run_i in range(actual_num_run):
        ...
        df_pick = df_log[df_log['MSG'] == 'AV_IDLE'].groupby(['hour', 'AV'], as_index=False).agg(REQ=("OTHER", "sum"))
        df_pick = df_pick.groupby(['hour'], as_index=False).agg(REQ=("REQ", "mean"))
    ...
    plt.plot(hour_list, pick_avg, label="avg. vehicle idle", alpha=1.0)
    #plt.plot(hour_list, drop_avg, label="avg. indiv. a.delay", alpha=0.7)
